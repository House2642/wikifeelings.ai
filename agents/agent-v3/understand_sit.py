...
def merge_dictionary(old: dict[str, float], new: dict[str,float]) -> dict[str,float]:
    if old is None:
        old = {}
    if new is None:
        return old
...
class Situation(BaseModel):
    ...
    automatic_thoughts: Annotated[dict[str, float], merge_dictionary] = Field(default_factory=dict, description="List of automatic thoughts with the confidence that is what the user is actually thinking or assuming")
    meaning_of_at: Optional[str] = Field(default=None, description="What these automatic thoughts mean to the person")
    emotions: Annotated[dict[str, float], merge_dictionary] = Field(default_factory=dict, description="Emotions experienced in this situation with a confidence score based on how confident we are the user is experiencing that emotion")
    behavior: Annotated[dict[str, float], merge_dictionary] = Field(default_factory=dict, description="Behaviors or actions taken in response and the confidence that they are leading to negative outcomes or effecting mood")
...
- rapid interpretations, predictions, mental images, negative, self-evaluations, assumptions about what others think, exaggerated catastrophe thoughts
