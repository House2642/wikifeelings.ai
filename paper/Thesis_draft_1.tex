...
Using widely-accepted criteria from the cognitive behavioral therapy (CBT) community, this work studies the quality of care and risk profiles of LLM-based therapists. Because automated treatment is new, and its risks are unknown, this work proposes a risk-free simulated patient we call a \emph{persona} as a kind of “animal model” for determining the efficacy of therapy sessions. Based on prior work that characterizes common traits among college students having mild to severe anxiety, we derive 14 different personas and place them in common anxiety-inducing social scenarios. A therapy agent is then tasked with treating a simulated patient of a persona and then evaluated against well-known evidence-based quality of care scales like the cogntivie therapy rating scale (CTRS).   We also place therapists in simulated crisis situations drawn from real-world dialogues taken from reddit, and evaluate their ability to detect and appropriately respond to dangerous events. 

Our work proposes a therapy agent framework called TheraEph. TheraEph uses a finite state machine (FSM) structure  progressive disclosure, and LLM monitor agents to guide a LLM through a CBT session. We also test a self improvement strategy that is derived from Anthropic's Constituion AI, to revise outputs to adhere to therapy principle on the CTRS. Each strategy is compared against a baseline therapy agent which is anthropics haiku with a system prompt detailing the model to act as a lead CBT therapist. 

TheraEph preformed the best averaging a CTRS score of 
...
\begin{table}[ht]
\centering
\caption{CTRS scores for the baseline therapist, run \texttt{4c025580} ($n = 14$ persona conversations; all values normalised as \% of scale maximum). 95\% CIs via $t$-distribution ($df = 13$).}
\label{tab:ctrs_baseline}
...
\hline
\textbf{Part I total} & \textbf{50.0} & \textbf{3.8} & $\mathbf{\pm 2.2\%}$ \\
\hline
\hline
...
\hline
\textbf{Part II total} & \textbf{67.6} & \textbf{5.1} & $\mathbf{\pm 3.0\%}$ \\
\hline
\hline
\textbf{CTRS total} & \textbf{58.0} & \textbf{3.1} & $\mathbf{\pm 1.8\%}$ \\
\hline
\end{tabular}
\end{table}
...
\begin{table}[ht]
\centering
\caption{CTRS scores for the bad baseline - Haiku, run \texttt{add7e6c3} ($n = 14$ persona conversations; all values normalised as \% of scale maximum). 95\% CIs via $t$-distribution ($df = 13$).}
...
\hline
\textbf{Part I total} & \textbf{19.6} & \textbf{8.6} & $\mathbf{\pm 5.0\%}$ \\
\hline
\hline
\multicolumn{4}{|l|}{\textbf{Part II --- Conceptualization, Strategy \& Technique}} \\
\hline
...
\hline
\textbf{Part II total} & \textbf{26.7} & \textbf{19.7} & $\mathbf{\pm 11.3\%}$ \\
\hline
\hline
\textbf{CTRS total} & \textbf{22.8} & \textbf{13.0} & $\mathbf{\pm 7.5\%}$ \\
\hline
\end{tabular}
\end{table}
\FloatBarrier
...
\begin{table}[ht]
\centering
\caption{CTRS scores for the TheraEph therapist, run \texttt{098bcce8} ($n = 14$ persona conversations; all values normalised as \% of scale maximum). 95\% CIs via $t$-distribution ($df = 13$).}
\label{tab:ctrs_final_monitor}
\begin{tabular}{|l|r|r|r|}
\hline
\textbf{Subscale} & \textbf{Mean (\%)} & \textbf{SD (\%)} & \textbf{95\% CI} \\
\hline
...
\hline
\textbf{Part I total} & \textbf{65.7} & \textbf{4.2} & $\mathbf{\pm 2.4\%}$ \\
\hline
\hline
\multicolumn{4}{|l|}{\textbf{Part II --- Conceptualization, Strategy \& Technique}} \\
\hline
...
\hline
\textbf{Part II total} & \textbf{74.8} & \textbf{3.9} & $\mathbf{\pm 2.2\%}$ \\
\hline
\hline
\textbf{CTRS total} & \textbf{69.8} & \textbf{3.3} & $\mathbf{\pm 1.9\%}$ \\
\hline
\end{tabular}
\end{table}
...
\begin{table}[ht]
\centering
\caption{CTRS scores for the Constitutional - Haiku, run \texttt{4202cf51} ($n = 14$ persona conversations; all values normalised as \% of scale maximum). 95\% CIs via $t$-distribution ($df = 13$).}
...
\hline
\textbf{Part I total} & \textbf{61.3} & \textbf{4.7} & $\mathbf{\pm 2.7\%}$ \\
\hline
\hline
\multicolumn{4}{|l|}{\textbf{Part II --- Conceptualization, Strategy \& Technique}} \\
\hline
...
\hline
\textbf{Part II total} & \textbf{69.3} & \textbf{4.9} & $\mathbf{\pm 2.8\%}$ \\
\hline
\hline
\textbf{CTRS total} & \textbf{64.9} & \textbf{4.4} & $\mathbf{\pm 2.5\%}$ \\
\hline
\end{tabular}
\end{table}
\FloatBarrier
