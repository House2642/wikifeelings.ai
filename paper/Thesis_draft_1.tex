...
\bottomrule
\end{tabular}
\caption{Therapy flow transitions for the Monitor Therapy Agent.}
...
\bottomrule
\end{tabular}
\caption{Crisis flow transitions for the Monitor Therapy Agent.}
...
\bottomrule
\end{tabular}
\caption{Input filtering transitions for the Monitor Therapy Agent.}
...
\section{Monitor Agent}

Just like baseline, we ran our ?agent? through 1 conversation with all 14 personas, and evaluated it on treatment fidelity, handling crisis, and case conceptualization. We will be comparing ?agent? to our baseline with the good prompt since it best simulates treatment a user would recieve from a basic chat with claude or chatgpt.

\subsection{Treatment Fidelity}
\begin{table}[ht]
\centering
\caption{CTRS scores for the final monitor therapist, run \texttt{098bcce8} ($n = 14$ persona conversations; all values normalised as \% of scale maximum). 95\% CIs via $t$-distribution ($df = 13$).}
...
\addlegendentry{Final monitor (\texttt{098bcce8})}
...
\end{axis}
\end{tikzpicture}
\caption{CTRS item scores (raw, 0--6 scale) for the final monitor (\texttt{098bcce8}) vs.\
...
Our agent does better overall with a total ctrs percentage of 69.8\% as compared to the baseline's 58.0\%. The biggest increase in performance for ?agent? from increases in agenda setting, eliciting patient feedback, gudided discovery, and homework. Additionally, ?agent? performs slightly better on collaboration, and understanding. 

\subsection{Crisis Handling}
\begin{table}[ht]
\centering
\caption{Crisis classification and response quality for the Monitor Agent - Crisis, run \texttt{f93af7c8} ($n = 40$ cases: 20 crisis, 20 no-crisis).}
...
The ?agent? has the same classification accurarcy of 95\% as our baseline agent. It performs significantly better at De-escalation, de-escalting 50\% of the time compared to 20\% on our baseline. Additionally, ?agent? requests human consultation 90\% of the time compared to only 10\% of the time for the baseline. However, ?agent? does worse on assesing the situation at 70\% compared to 90\% for our baseline. 

\subsection{Case Conceptualization}
\begin{table}[ht]
\centering
\caption{Conceptualization scores for the Final Monitor - Haiku CTRS, run \texttt{098bcce8} ($n = 14$ persona conversations; component scores normalised as \% of maximum). 95\% CIs via $t$-distribution ($df = 13$). Distortion overlap measured by Jaccard similarity (predicted vs.\ ground-truth distortion sets).}
...
\end{axis}
\end{tikzpicture}
\caption{CTRS item scores (raw, 0--6 scale) for the final monitor (\texttt{098bcce8}),
